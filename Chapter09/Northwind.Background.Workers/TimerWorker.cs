namespace Northwind.Background.Workers
{
    public class TimerWorker : IHostedService, IAsyncDisposable
    {
        ...
        private void DoWork(object? state)
        {
            ...
            _logger.LogInformation("TimerWorker {0} is working, execution count: {1:#,0}", nameof(TimerWorker), count);
        }

        public Task StartAsync(CancellationToken cancellationToken)
        {
            _logger.LogInformation("TimerWorker {0} is running.", nameof(TimerWorker));
        ...
        {
            _logger.LogInformation("TimerWorker {0} is stopping.", nameof(TimerWorker));
